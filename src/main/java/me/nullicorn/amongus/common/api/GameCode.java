...
public class GameCode {
  ...
  private static final byte[] V2_HEX_MAPPINGS  = new byte[]{
      0x19,
      0x15,
      0x13,
      0x0A,
  ...
  /**
   * @param data A 4-byte array that represents a game code
   */
  public GameCode(@NotNull byte[] data) {
    if (data.length != 4) {
      throw new IllegalArgumentException("Game code must be 4-bytes long");
    }

    version = new String(data, StandardCharsets.UTF_8).matches(V1_REGEXP)
        ? VERSION_1
        : VERSION_2;
  ...
  private static int v2CodeToInt(String codeStr) {
    byte b1 = V2_HEX_MAPPINGS[codeStr.charAt(0) - 65];
    byte b2 = V2_HEX_MAPPINGS[codeStr.charAt(1) - 65];
    byte b3 = V2_HEX_MAPPINGS[codeStr.charAt(2) - 65];
    byte b4 = V2_HEX_MAPPINGS[codeStr.charAt(3) - 65];
    byte b5 = V2_HEX_MAPPINGS[codeStr.charAt(4) - 65];
    byte b6 = V2_HEX_MAPPINGS[codeStr.charAt(5) - 65];
